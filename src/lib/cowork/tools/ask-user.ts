...
export const askUserQuestionTool: ToolExecutor = {
  ...
  description: "Ask the user a multi-choice question to clarify requirements or get input. Use this when you need user input before proceeding. The question will be displayed as an interactive card in the chat, and the agent will wait for the user's response.",
  parameters: {
    type: "object",
    properties: {
      prompt: {
      ...
      options: {
        ...
        items: {
          ...
          properties: {
            ...
            label: { type: "string", description: "Option label displayed to user" },
          },
          required: ["id", "label"],
        },
      ...
      questionId: {
        type: "string",
        description: "Optional question ID (generated by system if not provided)",
      },
    },
  ...
  async execute(input, context) {
    const { prompt, options, allowMultiple, questionId: providedQuestionId } = input as {
      prompt: string;
    ...
    const questionId = providedQuestionId || `question_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    try {
      ...
      const answerText = Object.entries(answers)
        .map(([questionId, answer]) => {
          ...
          return option ? `${option.label}: ${answer}` : `${questionId}: ${answer}`;
        })
        .join(", ");
