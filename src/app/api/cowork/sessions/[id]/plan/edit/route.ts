/**
 * Plan Edit API
 * POST /api/cowork/sessions/:id/plan/edit
 */
...
import { getPlan, removePlan, storePlan } from "@/lib/cowork/plan-store";
import { z } from "zod";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const { planId, editedPlan } = editPlanSchema.parse(body);
    ...
    // Edit: update plan and approve it
    ...
    plan.plan = editedPlan;
    plan.steps = steps;
    storePlan(plan); // Update stored plan
    ...
    return NextResponse.json({
      success: true,
      message: "Plan edited and approved. Execution can continue.",
    });
  } catch (error) {
