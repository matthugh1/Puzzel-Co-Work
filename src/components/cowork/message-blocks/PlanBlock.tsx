...
export function PlanBlock({
  ...
  const [editText, setEditText] = useState(steps.map((s) => s.description).join("\n"));
  ...
  const handlePlanAction = useCallback(
    async (action: "approve" | "reject") => {
      ...
      try {
        ...
        const res = await fetch(`/api/cowork/sessions/${sessionId}/plan/${action}`, {
          method: "POST",
          headers: {
        ...
        if (res.ok) {
          ...
          if (action === "approve") {
            dispatch({ type: "UPDATE_SESSION", payload: { id: sessionId, planMode: false } });
          }
        } else {
          const data = await res.json().catch(() => ({}));
          if (res.status === 404 && data.code === "PLAN_NOT_FOUND") {
            setActionError(
              data.error || "Plan expired or already resolved. Ask the assistant to propose a new plan.",
            );
          } else {
  ...
  return (
    <div className="cowork-interactive-card cowork-interactive-card--plan">
      ...
      {editing ? (
        <>
          ...
          <div className="cowork-interactive-card__actions cw-plan-actions">
            ...
            <button
              ...
              onClick={() => { setEditing(false); setEditText(steps.map((s) => s.description).join("\n")); }}
              disabled={loading}
            >
              Cancel
            </button>
        ...
        <>
      <ol className="cw-plan-list" style={{ margin: "8px 0", paddingLeft: 20, fontSize: "0.875rem", lineHeight: 1.7 }}>
        {steps.map((step, i) => (
          <li
            key={step.id || i}
            style={{
      ...
      <div className="cowork-interactive-card__actions">
        {actionError && (
          <p className="cw-plan-action-error" style={{ margin: "0 0 8px", fontSize: "0.8125rem", color: "var(--color-text-secondary)" }}>
            {actionError}
          </p>
        )}
        {resolved ? (
          ...
          <>
            ...
            <button
              className="cowork-interactive-card__btn"
              onClick={() => { setActionError(null); setEditing(true); setEditText(steps.map((s) => s.description).join("\n")); }}
              disabled={loading}
            >
              Edit
            </button>
