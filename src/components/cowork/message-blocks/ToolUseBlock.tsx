...
function toolSummary(
  ...
  if (command)
    return command.length > 45 ? command.slice(0, 42) + "…" : command;
  if (query) return query.length > 45 ? query.slice(0, 42) + "…" : query;
  if (pattern)
    return pattern.length > 45 ? pattern.slice(0, 42) + "…" : pattern;
    ...
    <div className="cowork-message__tool-block cowork-message__tool-block--use">
      ...
      <div className="cowork-tool-card cowork-tool-card--compact">
        ...
        >
          ...
          {summary != null && (
            <span className="cowork-tool-card__summary" title={summary}>
              • {summary}
            </span>
          )}
        </button>
